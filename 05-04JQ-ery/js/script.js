...
var asmenys = [];
var ams_list_string = localStorage.getItem("Asm_sarasas");
var Asm_sarasas = JSON.parse(ams_list_string);

if(Asm_sarasas != null){
    asmenys = Asm_sarasas;
}

printAsmenys(asmenys);
...
function printProduktai(produktai){
    ...
    h = h + "<tr><thead class='thead-dark'><th>Vardas</th> <th>Pavarde</th><th>Patiekalai</th></thead></tr>"
    for (var i = 0; i < asmenys.length; i++){
        let asm = asmenys[i];
        h = h + "<tr><td>" + asm.vardas + "</td>"
            + "<td>" + asm.pavarde + "</td>"
            + "<td>" + asm.patiekalai + "&euro;" + "</td>"
